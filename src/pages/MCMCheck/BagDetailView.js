...
const BagDetailView = () => {
  ...
  // 백엔드 데이터가 들어온 경우 진짜 recommendationProductId를 id로 사용
  const products = passedProducts && passedProducts.length > 0 ? passedProducts.map((p, index) => {
    const customSpecs = [
      { storage: "★★★★★", utility: "★★★★★", travel: "★★★★★", commute: "★★★★☆", laptop: "◯", airline: "◯" },
      { storage: "★★★★☆", utility: "★★★★★", travel: "★★★★☆", commute: "★★★★★", laptop: "◯", airline: "✕" },
      { storage: "★★★★★", utility: "★★★★☆", travel: "★★★☆☆", commute: "★★★★★", laptop: "◯", airline: "◯" }
    ];

    return {
      id: p.recommendationProductId, 
      title: p.name,
      image: p.imageUrl,
      homepageUrl: p.detailUrl || "https://kr.mcmworldwide.com",
      specs: customSpecs[index] || customSpecs[0],
      analysisText: p.recommendationReason
    };
  }) : [
    // 데이터가 없을 때 표시할 임시 UI (하드코딩된 데이터)
    {
      id: null, 
      ...
      analysisText: "높은 수납력과 뛰어난 활용도를 갖춘 백팩으로, 학교·업무·여행 등 다양한 상황에 적합합니다. 노트북 수납이 가능해 출퇴근용으로 활용하기 좋으며, 기내용 사이즈로 여행 시에도 편리하게 사용할 수 있습니다."
    },
    {
      id: null,
  ...
  const handleIssueVisitPass = () => {
    if (!currentProduct?.id) {
      alert("진단 결과 데이터가 정상적으로 전달되지 않았습니다. 진단을 다시 진행해주세요.");
      navigate('/');
      return;
    }
